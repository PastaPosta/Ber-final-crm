...
def edit(id):
    ...
    if form.validate_on_submit():
        ...
        if file:
            ...
            if '.' in filename and filename.rsplit('.', 1)[1].lower() in allowed_extensions:
                ...
                try:
                 ...
                 current_material = course.material_path
                 return render_template('courses/form.html', form=form, course=course, action="Edit", current_material=current_material)
        # --- Summarization Logic for EDIT ---
        # Run if material path changed and is PDF, or if it's an existing PDF without a summary (or with an error summary)
        if course.material_path and course.material_path.lower().endswith('.pdf'):
            needs_summary_update = False
            if material_path_changed: # New PDF uploaded or existing material changed to PDF
                needs_summary_update = True
            elif not course.material_summary or course.material_summary.startswith("Error:"): # no valid summary
                needs_summary_update = True
        ...
        elif course.material_summary: # Material is no longer a PDF -> clear existing summary
            current_app.logger.info(f"Material for course {course.id} is no longer a PDF. Clearing summary.")
            course.material_summary = None
            flash('Material is no longer a PDF; summary cleared.', 'info')
